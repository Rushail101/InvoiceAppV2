...
export async function backfillExpenseJournals(bizId) {
  // Get all expenses not yet journalled
  const { data: expenses } = await supabase.from('expenses')
    .select('*').eq('business_id', bizId)
    .or('journal_posted.is.null,journal_posted.eq.false');
  if (!expenses?.length) return 0;

  // Check which ones already have a journal via source_id
  const { data: existingJnls } = await supabase.from('journal_entries')
    .select('source_id').eq('business_id', bizId).eq('source', 'expense');
  const alreadyPosted = new Set((existingJnls || []).map(j => j.source_id));

  let count = 0;
  for (const exp of expenses) {
    if (alreadyPosted.has(exp.id)) continue;
    const acctName = CATEGORY_ACCOUNT_MAP[exp.category] || 'Miscellaneous';
    const expAcct = await findAccount(bizId, acctName);
    const cashAcct = await findAccount(bizId, 'Bank Account') ||
                     await findAccount(bizId, 'Cash');
    ...
    const { data: jnl, error: je } = await supabase.from('journal_entries').insert({
      ...
      narration: `${exp.category}${exp.description ? ' — ' + exp.description : ''}`,
      source: 'expense',
      source_id: exp.id,
    }).select().single();
    if (je) continue;
