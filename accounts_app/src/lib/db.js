...
export async function backfillExpenseJournals(bizId) {
  ...
  // Verify by ACTUAL journal_entries in DB — source_id is the truth, not journal_posted flag
  // journal_posted=true can be stale if old code set it before the journal was created
  const { data: existingJnls } = await supabase.from('journal_entries')
    .select('source_id').eq('business_id', bizId).eq('source', 'expense').not('source_id', 'is', null);

  const postedBySourceId = new Set((existingJnls || []).map(j => j.source_id));
...
export async function repairOrphanedExpenses(bizId) {
  ...
  const postedIds = new Set((existingJnls || []).map(j => j.source_id));
