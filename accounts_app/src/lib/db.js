...
export async function saveExpenseWithJournal(data) {
  // 1. Save expense WITHOUT journal_posted yet — set it only after journal is confirmed
  const { data: expRow, error } = await supabase.from('expenses')
    .insert({ ...data, vendor_id: data.vendor_id || null, journal_posted: false })
    .select().single();
  if (error) throw error;

  // 2. Find expense account (by category mapping) with fallback chain
  const acctName = CATEGORY_ACCOUNT_MAP[data.category] || 'Miscellaneous';
  const { data: allAccts } = await supabase.from('accounts')
    .select('*').eq('business_id', data.business_id);
  ...
  const cashAcct = (allAccts || []).find(a => /bank account/i.test(a.name)) ||
                   (allAccts || []).find(a => /\bcash\b/i.test(a.name)) ||
                   (allAccts || []).find(a => a.group === 'asset' && /current/i.test(a.sub_group || '')) ||
                   (allAccts || []).find(a => a.group === 'asset');

  if (!expAcct) throw new Error(`No expense account found for category "${data.category}". Please create a "${acctName}" account in Chart of Accounts.`);
  if (!cashAcct) throw new Error('No bank/cash account found. Please create a Bank Account in Chart of Accounts.');

  // 3. Create journal entry (Dr Expense / Cr Bank)
  // Use reference = EXP-<full-uuid> as the stable link — no source/source_id column needed
  const expRef = `EXP-${expRow.id}`;
  const { data: jnl, error: je } = await supabase.from('journal_entries').insert({
    ...
    narration: data.method || '',
  }).select().single();
  if (je) throw new Error(`Journal entry creation failed: ${je.message}`);
  ...
  // 4. Only mark as posted AFTER journal is confirmed written
...
export async function backfillExpenseJournals(bizId) {
  // Get ALL expenses for this business (ignore journal_posted flag — it may be stale/wrong)
  ...
  // Detect already-posted expenses by reference field: EXP-<expense-uuid>
  // This works without needing source/source_id columns in journal_entries
  const { data: existingJnls } = await supabase.from('journal_entries')
    .select('reference').eq('business_id', bizId).like('reference', 'EXP-%');

  const postedByRef = new Set((existingJnls || [])
    .map(j => j.reference?.replace('EXP-', ''))
    .filter(Boolean));
  ...
  for (const exp of expenses) {
    if (postedByRef.has(exp.id)) { skipped++; continue; }
...
// Repair expenses that have journal_posted=true but no actual journal entry in the DB.
// This happens when old code set journal_posted=true before the journal was created.
export async function repairOrphanedExpenses(bizId) {
  const { data: flagged } = await supabase.from('expenses')
    .select('id').eq('business_id', bizId).eq('journal_posted', true);
  if (!flagged?.length) return { repaired: 0 };

  const { data: existingJnls } = await supabase.from('journal_entries')
    .select('reference').eq('business_id', bizId).like('reference', 'EXP-%');

  const postedIds = new Set((existingJnls || [])
    .map(j => j.reference?.replace('EXP-', ''))
    .filter(Boolean));
  const orphaned = flagged.filter(e => !postedIds.has(e.id)).map(e => e.id);

  if (!orphaned.length) return { repaired: 0 };

  // Reset journal_posted=false so backfill will pick them up
  await supabase.from('expenses').update({ journal_posted: false }).in('id', orphaned);
