...
import { saveParty, deleteParty, saveExpense, saveExpenseWithJournal, backfillExpenseJournals, deleteExpense, savePayment, saveBankAccount, saveBankTxn, deleteBankTxn, deletePayment, updateInvoiceStatus, saveInvoice } from '../lib/db.js';
...
export function ExpensesView({ expenses, businesses, parties, activeBiz, reload, accounts, journalEntries }) {
  ...
  async function backfill() {
    ...
    setBackfilling(true);
    const count = await backfillExpenseJournals(bizId);
    setBackfilling(false);
    reload();
    alert(count > 0 ? `✅ Posted ${count} journal entries.` : 'All expenses are already journalled.');
  }
