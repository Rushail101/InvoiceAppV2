...
import { saveParty, deleteParty, saveExpense, deleteExpense, savePayment, saveBankAccount, saveBankTxn, deleteBankTxn, deletePayment, updateInvoiceStatus } from '../lib/db.js';
...
function PartyModal({ onClose, onSave, businesses, editData }) {
  const [f, setF] = useState({
    ...
    gstin: editData?.gstin || '', state: editData?.state || '',
...
// ─── EXPENSES VIEW ────────────────────────────────────────────────────────────
export function ExpensesView({ expenses, businesses, parties, activeBiz, reload }) {
  const [search, setSearch] = useState('');
  const [showModal, setShowModal] = useState(false);
  ...
  async function save(data) { await saveExpense(data); reload(); }
  ...
  return (
    <>
      ...
      <div className="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Vendor</th><th>Method</th><th>Ref</th><th className="r">Amount</th><th></th></tr></thead>
          <tbody>
            {filtered.map(e => (
              <tr key={e.id}>
            ...
            {filtered.length === 0 && <tr><td colSpan={8}><EmptyState icon="💸" message="No expenses" /></td></tr>}
          </tbody>
        </table>
      </div>
      {showModal && <ExpenseModal onClose={() => setShowModal(false)} onSave={save} businesses={businesses} parties={parties} />}
    </>
  );
}

function ExpenseModal({ onClose, onSave, businesses, parties }) {
  const [f, setF] = useState({ business_id: businesses[0]?.id || '', category: '', description: '', amount: '', expense_date: today(), vendor_id: '', method: 'UPI', reference: '' });
  const [busy, setBusy] = useState(false);
  const vendors = parties.filter(p => p.type === 'vendor' && p.business_id === f.business_id);
  async function save() { if (!f.category || !f.amount) return; setBusy(true); await onSave(f); setBusy(false); onClose(); }
  return (
...
function BankModal({ onClose, onSave, businesses, activeBiz }) {
  const [f, setF] = useState({ business_id: activeBiz || businesses[0]?.id || '', name: '', bank_name: '', account_number: '', ifsc: '', opening_balance: 0 });
  const [busy, setBusy] = useState(false);
  async function save() { if (!f.name.trim()) return; setBusy(true); await onSave(f); setBusy(false); onClose(); }
  return (
    <ModalShell title="Add Bank Account" onClose={onClose}
      ...
      <div className="form-row cols-2">
        ...
        <FG label="IFSC"><input value={f.ifsc} onChange={e => setF(x => ({ ...x, ifsc: e.target.value }))} /></FG>
      </div>
      <div className="form-row cols-2">
...
function BankTxnModal({ onClose, onSave, bankAccountId }) {
  ...
  const [busy, setBusy] = useState(false);
  async function save() { if (!f.description || !f.amount) return; setBusy(true); await onSave({ ...f, amount: Number(f.amount) }); setBusy(false); onClose(); }
  return (
