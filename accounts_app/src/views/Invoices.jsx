...
import { fmt, fmtDate, today, GST_RATES, PAY_MODES, INDIAN_STATES, gstType, calcLineTax, nextInvNum } from '../lib/constants.js';
...
export function PaymentModal({ onClose, onSave, invoice, existingPayments }) {
  ...
  return (
    <ModalShell title="Record Payment" onClose={onClose}
      foot={<><button className="btn btn-ghost" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={save} disabled={busy}>{busy ? 'Saving…' : 'Record Payment'}</button></>}>
      <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 10 }}>
        Invoice <span style={{ color: 'var(--accent)', fontFamily: 'var(--mono)' }}>{invoice.invoice_number}</span> — Total {fmt(invoice.total)}
      </p>
                ...
                <tr key={inv.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                      {inv.status === 'proforma' && <button className="btn btn-warning btn-sm" onClick={() => convertProforma(inv)}>→ Invoice</button>}
                      {!['paid', 'cancelled', 'proforma'].includes(inv.status) && <button className="btn btn-ghost btn-sm" onClick={() => setPayInv(inv)}>💰</button>}
