...
// ── Component ──────────────────────────────────────────────────────────────────
export function BulkImportView({ bankAccounts, accounts, invoices, payments, parties, activeBiz, reload }) {
  ...
  async function handleParseAll() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        for (const t of txns) {
          ...
          seen.add(k);
          parsed.push({ ...t, cls: classify(t), bankAccountId: bankAccId, approved: false, skip: false });
        }
    ...
    parsed = parsed.map(t => ({ ...t, approved: t.cls.confidence === 'high' }));

    // Build unclassified groups for batch assignment
    const unclassified = parsed.filter(t => t.cls.confidence === 'low');
    const groupMap = {};
