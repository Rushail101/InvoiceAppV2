...
import { matchBankToPayments, matchBankToJE } from './Reconciliation.jsx';
...
// ── Component ──────────────────────────────────────────────────────────────────
export function BulkImportView({ bankAccounts, bankTransactions = [], accounts, invoices, payments, parties, journalEntries = [], journalLines = [], activeBiz, reload }) {
  ...
  async function handleParseAll() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        for (const t of txns) {
          ...
          parsed.push({
            ...t, id: k, cls: classify(t), bankAccountId: bankAccId,
    ...
    // Cross-check against payments/journal entries you've already recorded
    // manually (e.g. via the Payments screen) — these aren't in the
    // bank_transactions table yet so findExistingDuplicate above can't see
    // them, but posting them again here would double the journal entry.
    // Reuses the exact matching Reconciliation uses, so "already in books"
    // here means the same thing it means there.
    const notDupe = parsed.filter(t => !t.dupe);
    const payMatches = matchBankToPayments(notDupe, payments, invoices, parties);
    const payMatchedIds = new Set(payMatches.map(m => m.bankTxn.id));
    const jeMatches = matchBankToJE(notDupe, journalEntries || [], journalLines || [], accounts, payMatchedIds);
    const bookMatchDetail = {};
    payMatches.forEach(m => { bookMatchDetail[m.bankTxn.id] = `Payment · ${m.party?.name || ''} · ${m.invoice?.invoice_number || ''}`; });
    jeMatches.forEach(m => { bookMatchDetail[m.bankTxn.id] = `Journal entry · ${m.match.narration || m.match.reference || ''}`; });
    parsed = parsed.map(t => bookMatchDetail[t.id]
      ? { ...t, alreadyInBooks: true, bookMatchDetail: bookMatchDetail[t.id], approved: false, skip: true }
      : t);
    ...
    // or transactions already matched to a recorded payment/journal entry)
    parsed = parsed.map(t => ({ ...t, approved: !t.dupe && !t.alreadyInBooks && t.cls.confidence === 'high' }));

    // Build unclassified groups for batch assignment — duplicates and
    // already-in-books rows are already excluded, so leave them out of the
    // grouping workflow entirely.
    const unclassified = parsed.filter(t => t.cls.confidence === 'low' && !t.dupe && !t.alreadyInBooks);
    const groupMap = {};
  ...
  const alreadyInBooks   = allTxns.filter(t => t.alreadyInBooks);
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
            {[
              ...
              { label: 'Already in books', val: alreadyInBooks.length, color: '#5ca0ff' },
          ...
          {/* Already in books — matched to a payment or journal entry you already
              recorded manually. Shown for transparency, never auto-posted again. */}
          {alreadyInBooks.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#5ca0ff', marginBottom: 10 }}>
                ℹ {alreadyInBooks.length} transaction{alreadyInBooks.length !== 1 ? 's' : ''} already in your books — won't be posted again
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {alreadyInBooks.map((t, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '90px 1fr auto auto', gap: 10, padding: '8px 12px', background: 'var(--bg2)', borderRadius: 7, border: '1px solid #16324f', alignItems: 'center' }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)' }}>{t.date}</div>
                    <div>
                      <div style={{ fontSize: 12, color: 'var(--text1)' }}>{t.description}</div>
                      <div style={{ fontSize: 11, color: 'var(--text3)' }}>{t.bookMatchDetail}</div>
                    </div>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 99, background: '#0a1a2f', border: '1px solid #16324f', color: '#5ca0ff' }}>Matched</span>
                    <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, color: t.type === 'credit' ? '#4ade80' : '#f87171', textAlign: 'right', minWidth: 80 }}>
