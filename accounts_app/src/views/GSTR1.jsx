...
// ─── HSN SUMMARY ─────────────────────────────────────────────────────────────
function buildHSNRows(invoices, allItems) {
  const hsnMap = {};
...
function HSNGrid({ rows, label, accent }) {
  if (!rows.length) return (
    <div style={{ padding: '12px 0', color: 'var(--text3)', fontSize: 12 }}>No {label} HSN data for this period.</div>
  );
  return (
    <div className="table-wrap">
      <table>
        ...
        <tfoot>
          <tr style={{ fontWeight: 700, background: 'var(--bg3)', borderTop: '2px solid var(--border2)' }}>
            ...
            <td className="r mono" style={{ padding: '9px 16px' }}>{fmt(rows.reduce((s, r) => s + r.cgst + r.sgst + r.igst, 0))}</td>
          </tr>
        </tfoot>
      </table>
    </div>
...
function HSNTable({ b2bInvoices, b2cInvoices, allItems }) {
  const [hsnMode, setHsnMode] = useState('b2b');
  const b2bRows = buildHSNRows(b2bInvoices, allItems);
  const b2cRows = buildHSNRows(b2cInvoices, allItems);

  if (!b2bRows.length && !b2cRows.length)
    return <EmptyState icon="🏷️" message="No HSN data available" sub="HSN codes on invoice line items will appear here" />;
        ...
        {[
          ...
          <button key={m.id} onClick={() => setHsnMode(m.id)}
            style={{
              ...
              background: hsnMode === m.id ? 'var(--accent)' : 'var(--bg2)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          ...
          {viewMode === 'collections' && (
            <div style={{ fontSize: 11, color: 'var(--amber)', fontFamily: 'var(--mono)', marginTop: 2 }}>
              ⚠️ Collections view — shows invoices paid this month, not the GSTR-1 compliance view
            </div>
          )}
        </div>
        {/* Mode toggle */}
