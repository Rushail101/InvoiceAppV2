...
function buildMonthlyFlow(journalEntries, journalLines, accounts, activeBiz, year) {
  ...
  for (const je of relevantJEs) {
    ...
    // Cash movement = net change on Bank account line. journal_lines uses
    // a `type` ('debit'/'credit') + `amount` column, not separate debit/
    // credit numeric columns — this was reading nonexistent fields, so
    // cashIn and cashOut were always 0 regardless of the real transaction.
    const cashIn  = bankLine.type === 'debit'  ? Number(bankLine.amount || 0) : 0;
    const cashOut = bankLine.type === 'credit' ? Number(bankLine.amount || 0) : 0;
