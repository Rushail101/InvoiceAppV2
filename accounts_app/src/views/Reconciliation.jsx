...
import { saveJournal } from '../lib/db.js';
...
export function matchBankToPayments(bankTxns, payments, invoices, parties) {
  const matched = [];
...
export function matchBankToJE(bankTxns, journalEntries, journalLines, accounts, matchedBankIds) {
  const matched = [];
...
export function ReconciliationView({
  ...
  payments, invoices, parties, accounts, activeBiz, reload
}) {
    ...
    <div style={{ maxWidth: 960, margin: '0 auto' }}>
      ...
      {tab === 'unmatched' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {unmatchedBank.map((t, i) => (
            <UnmatchedRow key={i} txn={t} bankAcct={bankAcct} bizAccounts={bizAccounts}
...
// An unmatched bank transaction with an inline "add to journal" action, so a
// missing entry can be posted right from this page instead of switching to
// Journal Vouchers. Uses the same Bank Account ledger leg Reconciliation's
// own matching (matchBankToJE) looks for, so the row disappears from
// Unmatched and shows up as Matched the next time this runs.
function UnmatchedRow({ txn, bankAcct, bizAccounts, activeBiz, reload }) {
  ...
  const otherAccounts = bizAccounts.filter(a => a.id !== bankAcct?.id);

  async function post() {
    if (!bankAcct) { setErr('No "Bank Account" ledger account found for this business.'); return; }
    if (!acctId) { setErr('Pick the other account for this entry.'); return; }
    setBusy(true); setErr('');
    try {
      const isCredit = txn.type === 'credit';
      const entry = {
        business_id: activeBiz, entry_date: txn.txn_date || txn.date,
        reference: '', description: narration || txn.description || 'Bank transaction', narration: '',
      };
      const lines = [
        { account_id: bankAcct.id, type: isCredit ? 'debit' : 'credit', amount: Number(txn.amount), narration },
        { account_id: acctId, type: isCredit ? 'credit' : 'debit', amount: Number(txn.amount), narration },
      ];
  ...
  return (
    <div style={{ background: 'var(--bg2)', borderRadius: 7, border: '1px solid #3a1a0a', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr auto auto', gap: 10, padding: '9px 12px', alignItems: 'center' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)' }}>{txn.txn_date || txn.date}</div>
        <div style={{ fontSize: 12.5, color: 'var(--text1)' }}>{txn.description}</div>
        <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, color: txn.type === 'credit' ? '#4ade80' : '#f87171', textAlign: 'right', minWidth: 90 }}>
          {txn.type === 'credit' ? '+' : '-'}{fmt(txn.amount)}
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => setOpen(o => !o)}>
          {open ? 'Cancel' : '+ Add to Journal'}
        </button>
      </div>
      {open && (
        <div style={{ padding: '10px 12px 14px', borderTop: '1px solid var(--border2)', display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ fontSize: 11, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>
              {txn.type === 'credit' ? 'Credit this account (source)' : 'Debit this account (expense/purpose)'}
            </label>
            <select value={acctId} onChange={e => setAcctId(e.target.value)}
              style={{ width: '100%', padding: '6px 10px', borderRadius: 6, background: 'var(--bg1)', border: '1px solid var(--border2)', color: 'var(--text1)', fontSize: 13 }}>
              <option value="">Select account…</option>
              {otherAccounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </div>
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ fontSize: 11, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>Narration</label>
            <input value={narration} onChange={e => setNarration(e.target.value)}
              style={{ width: '100%', padding: '6px 10px', borderRadius: 6, background: 'var(--bg1)', border: '1px solid var(--border2)', color: 'var(--text1)', fontSize: 13 }} />
          </div>
          <button className="btn btn-primary btn-sm" onClick={post} disabled={busy}>{busy ? 'Posting…' : 'Post Entry'}</button>
        </div>
      )}
      {err && <div style={{ padding: '0 12px 10px', fontSize: 11, color: '#f87171' }}>{err}</div>}
    </div>
  );
}
