...
// ══════════════════════════════════════════════════════════════════════════════
export async function parseICICIXls(file) {
...
// ══════════════════════════════════════════════════════════════════════════════
// DUPLICATE DETECTION — check against transactions already saved for this
// bank account (from a previous import or manual entry) so re-uploading the
// same statement (or an overlapping date range) doesn't create double entries.
// ══════════════════════════════════════════════════════════════════════════════
function findExistingDuplicate(txn, existingTxns) {
  if (!existingTxns?.length) return null;

  // Strongest signal: the bank's own transaction ID/reference matches exactly.
  // ICICI txn IDs are unique per transaction, so this alone is conclusive.
  if (txn.reference) {
    const refMatch = existingTxns.find(e => e.reference && e.reference === txn.reference);
    if (refMatch) return { reason: 'Already imported — same transaction ID' };
  }

  // Fallback for rows without a usable reference (or manually-entered txns):
  // same date + same amount + same direction is a very strong duplicate signal
  // for a single bank account.
  const fuzzyMatch = existingTxns.find(e =>
    e.txn_date === txn.date &&
    Number(e.amount) === Number(txn.amount) &&
    e.type === txn.type
  );
  if (fuzzyMatch) return { reason: 'Already recorded — same date & amount' };
