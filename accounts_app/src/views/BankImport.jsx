...
export function BankImportModal({
  ...
  const [jeResults, setJeResults] = useState([]); // per-row JE outcome after posting
  ...
  async function handlePost() {
    ...
    for (let i = 0; i < toPost.length; i++) {
      ...
      try {
        // 1. Save bank transaction + auto-generate journal entry
    ...
    setProgress(`✅ Posted ${posted} transactions · ${jePosted} journal entries created · ${jeSkipped} entries need manual accounts.`);
    setStep('done');
    reload();
  }
    ...
    >
      ...
      {step === 'done' && (
        <div style={{ padding: '24px 0' }}>
          ...
          {jeResults.length > 0 && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
                Journal Entry Log
              </div>
              <div style={{ maxHeight: '45vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
                {jeResults.map(({ row, result, error: rowErr }, i) => {
                  const hasJE = result?.journalId;
                  const isSkipped = result?.skipped;
                  const method = result?.mapping?.method;
                  const debit = result?.mapping?.debitAcct;
                  const credit = result?.mapping?.creditAcct;
                  const conf = result?.mapping?.confidence;
                  return (
                    <div key={i} style={{
                      display: 'grid', gridTemplateColumns: '80px 1fr auto',
                      gap: 10, alignItems: 'center',
                      padding: '8px 12px', borderRadius: 6,
                      background: rowErr ? '#1f0a0a' : hasJE ? '#0a1a0f' : '#1a1a0a',
                      border: `1px solid ${rowErr ? '#5c1a1a' : hasJE ? '#1a4a2a' : '#3a3a10'}`,
                    }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)' }}>{row.date}</div>
                      <div>
                        ...
                        {hasJE && (
                          <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>
                            Dr <span style={{ color: '#60a5fa' }}>{debit}</span>
                            {' / '}
                            Cr <span style={{ color: '#a78bfa' }}>{credit}</span>
                            {' '}
                            <span style={{
                              padding: '1px 6px', borderRadius: 99, fontSize: 10,
                              background: method === 'rule' ? '#0d2b1a' : '#1a1a2b',
                              border: `1px solid ${method === 'rule' ? '#1a5c36' : '#2a2a5c'}`,
                              color: method === 'rule' ? '#4ade80' : '#a78bfa',
                            }}>
                              {method === 'rule' ? '⚡ rule' : method === 'ai' ? '🤖 AI' : '⚠ fallback'}
                              {conf && ` · ${conf}`}
                            </span>
                          </div>
                        )}
                        {isSkipped && <div style={{ fontSize: 11, color: '#fbbf24' }}>⚠ No matching accounts found — add accounts in Chart of Accounts</div>}
                        {rowErr && <div style={{ fontSize: 11, color: '#f87171' }}>Error: {rowErr}</div>}
                      </div>
                      <div style={{
                        fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12,
                        color: row.type === 'credit' ? '#4ade80' : '#f87171',
                        textAlign: 'right',
                      }}>
