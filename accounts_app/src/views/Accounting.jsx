...
export function BalanceSheetView({ accounts, journalLines, journalEntries, invoices, payments, expenses, businesses, activeBiz }) {
  ...
  function getBalance(accountId) {
    ...
    return dr - cr; // positive = Dr balance
  }
  ...
  function AccSection({ title, accounts: accs, grp, color }) {
  ...
  return (
    <div>
      ...
      {bizList.map(biz => {
        ...
        const collected = bi.reduce((s, i) => s + (paysByInv[i.id] || []).reduce((ps, p) => ps + Number(p.amount), 0), 0);
        const outstanding = bi.reduce((s, i) => {
