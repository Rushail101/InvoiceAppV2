...
export function JournalView({ journalEntries, journalLines, accounts, businesses, activeBiz, reload }) {
  ...
  // the other one.
  const byId = {};
  bizFiltered.forEach(e => { byId[e.id] = e; });
  const reversalByOriginal = {};
  bizFiltered.forEach(e => { if (e.source === 'reversal' && e.source_id) reversalByOriginal[e.source_id] = e; });

  function applyPreset(preset) {
    const now = new Date();
    const pad = n => String(n).padStart(2, '0');
    const iso = d => `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
    if (preset === 'today') { setDateFrom(iso(now)); setDateTo(iso(now)); }
    else if (preset === 'this_month') { setDateFrom(iso(new Date(now.getFullYear(), now.getMonth(), 1))); setDateTo(iso(now)); }
  ...
  const filtered = bizFiltered.filter(j => {
    if (dateFrom && j.entry_date < dateFrom) return false;
    if (dateTo && j.entry_date > dateTo) return false;
    const src = j.source || 'manual';
    if (sourceFilter && src !== sourceFilter) return false;
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      const lines = linesByJournal[j.id] || [];
      const hay = [j.description, j.reference, ...lines.map(l => l.narration), ...lines.map(l => acctName(l.account_id))]
  ...
  async function handleSave(entry, lines, id) {
    await saveJournal(entry, lines, id);
    reload();
  }
  ...
  // One row per journal entry — good for a quick scan of what's there and
  // spotting gaps in the JV number sequence (e.g. JV-030 then JV-032 with no
  // JV-031 anywhere tells you something's missing).
  function exportSummaryCSV() {
    const rows = filtered.map(j => {
      ...
      return [j.entry_date, j.reference || '', SOURCE_LABEL[j.source || 'manual'] || j.source, j.description, totalDr.toFixed(2), totalCr.toFixed(2), lines.length];
    });
    download(toCSV(['Date', 'Reference', 'Source', 'Description', 'Debit Total', 'Credit Total', 'Line Count'], rows), `journal_summary_${today()}.csv`);
  }

  // One row per ledger line — the actual audit-trail view, showing exactly
  // which account each amount hit. Cross-check against your bank statement
  // or Chart of Accounts to catch anything that didn't get posted.
  function exportDetailCSV() {
    const rows = [];
    filtered.forEach(j => {
      const lines = linesByJournal[j.id] || [];
      lines.forEach(l => {
        rows.push([j.entry_date, j.reference || '', SOURCE_LABEL[j.source || 'manual'] || j.source, j.description, acctName(l.account_id), l.type === 'debit' ? 'Dr' : 'Cr', Number(l.amount).toFixed(2), l.narration || '']);
      });
    });
    download(toCSV(['Date', 'Reference', 'Source', 'Entry Description', 'Account', 'Dr/Cr', 'Amount', 'Line Narration'], rows), `journal_detail_${today()}.csv`);
  }
  ...
  function openReverse(j) { setModalMode('reverse'); setModalEntry({ entry: j, lines: linesByJournal[j.id] || [] }); setShowModal(true); }

  const filteredTotal = filtered.reduce((s, j) => s + (linesByJournal[j.id] || []).filter(l => l.type === 'debit').reduce((s2, l) => s2 + Number(l.amount), 0), 0);
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, marginTop: -8 }}>
        {[['today', 'Today'], ['this_month', 'This month'], ['last_month', 'Last month'], ['this_fy', 'This FY'], ['', 'Clear dates']].map(([id, label]) => (
        ...
        <table>
          ...
          <tbody>
            {filtered.map(j => {
              ...
              const showLines = expanded ? lines : lines.slice(0, 2);
              const src = j.source || 'manual';
              ...
              return (
                <tr key={j.id}>
                  ...
                  <td style={{ maxWidth: 320 }}>
                    ...
                    {reversalOf && (
                      <div className="tag" style={{ color: 'var(--purple)', borderColor: '#201848', marginTop: 3, display: 'inline-block' }}>↩ reverses {reversalOf.reference || 'entry'}</div>
                    )}
                    {reversedBy && (
                      <div className="tag" style={{ color: 'var(--amber)', borderColor: '#3a2e00', marginTop: 3, display: 'inline-block' }}>⚠ reversed by {reversedBy.reference || 'entry'}</div>
                    )}
                    {showLines.map((l, i) => (
                      <div key={i} style={{ fontSize: 10.5, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>{l.type === 'debit' ? 'Dr' : '  Cr'} {acctName(l.account_id)}</div>
                    ))}
                    {lines.length > 2 && (
                      <button className="btn btn-ghost btn-sm" style={{ padding: '1px 6px', marginTop: 3, fontSize: 10 }} onClick={() => setExpandedId(expanded ? null : j.id)}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <button className="btn btn-ghost btn-sm" onClick={() => openDuplicate(j)}>Copy</button>
                      {!reversedBy && <button className="btn btn-ghost btn-sm" onClick={() => openReverse(j)} title="Post a mirror entry that cancels this one out">Reverse</button>}
...
function JournalModal({ onClose, onSave, accounts, businesses, activeBiz, mode = 'new', existing, allEntries = [] }) {
  ...
  const [f, setF] = useState({
    business_id: srcEntry?.business_id || activeBiz || businesses[0]?.id || '',
    entry_date: isEdit && srcEntry ? srcEntry.entry_date : today(),
    reference: isEdit ? (srcEntry?.reference || '') : (isReverse && srcEntry ? `Reversal of ${srcEntry.reference || ''}`.trim() : ''),
    description: isReverse && srcEntry ? `Reversal — ${srcEntry.description || ''}` : (srcEntry?.description || ''),
    narration: srcEntry?.narration || '',
  });
  const [lines, setLines] = useState(
    srcLines.length
      ? srcLines.map(l => ({ account_id: l.account_id, type: isReverse ? (l.type === 'debit' ? 'credit' : 'debit') : l.type, amount: String(l.amount), narration: l.narration || '' }))
      : [
  ...
  function addLine() {
    setLines(p => [...p, { account_id: '', type: totalDr > totalCr ? 'credit' : 'debit', amount: '', narration: '' }]);
  }

  // Auto-balance: if exactly one line (with an account chosen) is missing an
  // amount, figure out what it needs to be for the entry to balance, and
  // fill it in automatically once the user tabs/clicks away from that field.
  function impliedAmount(idx) {
    const line = lines[idx];
    if (!line.account_id || line.amount !== '') return null;
    const emptyCount = lines.filter(l => l.account_id && l.amount === '').length;
    if (emptyCount !== 1) return null;
    const need = line.type === 'debit' ? (totalCr - totalDr) : (totalDr - totalCr);
    return need > 0.004 ? need : null;
  }
  ...
  async function save() {
    ...
    setErr(''); setBusy(true);
    try {
      const entryPayload = isReverse && srcEntry ? { ...f, source: 'reversal', source_id: srcEntry.id } : f;
      await onSave(entryPayload, validLines.map(l => ({ ...l, amount: Number(l.amount) })), isEdit ? srcEntry.id : undefined);
      onClose();
  ...
  function onNarrationKeyDown(e, idx) {
    if (e.key !== 'Enter') return;
    e.preventDefault();
    if (idx === lines.length - 1) {
      addLine();
      setTimeout(() => lineRefs.current[idx + 1]?.focus(), 20);
    } else {
  ...
  return (
    <ModalShell title={isEdit ? 'Edit Journal Entry' : mode === 'duplicate' ? 'Duplicate Journal Entry' : isReverse ? 'Reverse Journal Entry' : 'New Journal Entry'} onClose={onClose} size="modal-lg"
      foot={<>
        ...
        <button className="btn btn-primary" onClick={save} disabled={busy || !balanced}>{busy ? 'Posting…' : isEdit ? 'Save Changes' : isReverse ? 'Post Reversal' : 'Post Entry'}</button>
      </>}>
      <div onKeyDown={onModalKeyDown}>
        ...
        {lines.map((l, idx) => {
          const implied = impliedAmount(idx);
          return (
            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '2fr 80px 120px 1fr 28px', gap: 5, marginBottom: 6, alignItems: 'center' }}>
              <AccountSelect accounts={filtAccs} value={l.account_id} onChange={id => updLine(idx, 'account_id', id)} />
              ...
              <input
                ...
                onChange={e => updLine(idx, 'amount', e.target.value)}
                onBlur={() => { if (implied != null) updLine(idx, 'amount', implied.toFixed(2)); }}
                placeholder={implied != null ? `auto ${fmt(implied)}` : '0.00'}
                style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', color: 'var(--text)', borderRadius: 'var(--r)', padding: '6px 7px', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none', width: '100%' }}
              />
              <input
                ref={el => lineRefs.current[idx] = el}
                value={l.narration}
                onChange={e => updLine(idx, 'narration', e.target.value)}
                onKeyDown={e => onNarrationKeyDown(e, idx)}
                placeholder="Narration · Enter for next line"
                style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', color: 'var(--text)', borderRadius: 'var(--r)', padding: '6px 7px', fontSize: 12, outline: 'none', width: '100%' }}
              />
              {lines.length > 2 ? <button className="remove-btn" onClick={() => setLines(p => p.filter((_, i) => i !== idx))}>×</button> : <div />}
            </div>
          );
        })}
        <button className="btn btn-ghost btn-sm" style={{ marginTop: 4 }} onClick={() => addLine()}>+ Add line</button>
        {err && <p className="err-msg" style={{ marginTop: 8 }}>{err}</p>}
        <p style={{ fontSize: 10, color: 'var(--text4)', fontFamily: 'var(--mono)', marginTop: 10 }}>Tip: press Enter in a narration field to add the next line · ⌘/Ctrl+Enter to post</p>
      </div>
    </ModalShell>
  );
}
...
export function BalanceSheetView({ accounts, journalLines, journalEntries, invoices, payments, expenses, businesses, activeBiz }) {
  ...
  // Net income: use ALL accounts to avoid missing income/expense due to business_id mismatch
  const income = accounts.filter(a => a.group === 'income').reduce((s, a) => s - getBalance(a.id), 0);
  const expAmt = accounts.filter(a => a.group === 'expense').reduce((s, a) => s + getBalance(a.id), 0);
