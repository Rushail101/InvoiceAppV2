...
function Dashboard({ invoices, expenses, payments, businesses, parties, activeBiz, setView }) {
  ...
  const collected = saleInvs.reduce((s, i) => s + (paysByInv[i.id]||[]).reduce((ps,p)=>ps+Number(p.amount),0), 0);
  const pending = saleInvs.filter(i => ['sent','partially_paid'].includes(i.status)).reduce((s,i)=>s+Number(i.total),0);
  ...
  const monthlyData = (() => {
    ...
    for (let m = 5; m >= 0; m--) {
      ...
      const value = saleInvs
        .filter(i => { const id = new Date(i.issue_date); return id.getFullYear()===yr && id.getMonth()===mo; })
  ...
  saleInvs.forEach(i => {
    ...
    if (p) clientRevenue[p.name] = (clientRevenue[p.name] || 0) + Number(i.subtotal || 0);
  });
