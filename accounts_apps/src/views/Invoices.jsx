...
export function InvoicesView({ invoices, businesses, parties, activeBiz, reload, payments, creditNotes, catalogItems = [] }) {
  ...
  const filtered = invoices.filter(i => {
    ...
    // Extract numeric part from invoice numbers like 25-26/1001 or PI-25-26/001
    const getNum = inv => {
      const m = (inv.invoice_number || '').match(/(\d+)$/);
      return m ? parseInt(m[1], 10) : 0;
    };
    const getFYPart = inv => (inv.invoice_number || '').match(/(\d{2}-\d{2})/) ?.[1] || '';
    const fyDiff = getFYPart(b).localeCompare(getFYPart(a)); // latest FY first
    if (fyDiff !== 0) return fyDiff;
    return getNum(b) - getNum(a); // highest number first within same FY
  });
  ...
  return (
    <>
      <div className="filter-bar">
        ...
        <button className="btn btn-ghost btn-sm" onClick={autoNumber} title="Sort all invoices by date and assign sequential numbers per FY (e.g. 25-26/1001)">⟳ Auto-Number</button>
